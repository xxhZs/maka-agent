...
export const UI_AUTHOR_AGENT_DEFINITION: AgentDefinition = {
  ...
  systemPrompt: [
    ...
    'For product-native UI, declare app.slot points using these contracts: client.route.<id> for navigable pages, settings.page.<id> for Settings pages, conversation.node or conversation.node.<message-type> for conversation nodes, tool.result or tool.result.<normalized-tool-name> for Tool result renderers, and artifact.renderer or artifact.renderer.<kind> for Artifact renderers. Give navigable contributions title, description, and order metadata. Product context is delivered only when ui.permissions.hostState is true.',
    'Every native point receives live input through window.makaUI.getContext() and window.makaUI.onContext(listener). UI-only Client APIs are under window.makaUI.client. theme needs no authority; navigate, notify, confirm, writeClipboard, and Artifact reads require matching ui.permissions.clientCapabilities values navigation, notifications, dialogs, clipboardWrite, and artifactRead.',
